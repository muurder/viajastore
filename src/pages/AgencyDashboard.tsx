...
const TransportManager: React.FC<{ trip: Trip; bookings: Booking[]; clients: any[]; onSave: (data: OperationalData) => void }> = ({ trip, bookings, clients, onSave }) => {
    ...
    // Merge booking passengers + manual passengers
    const allPassengers = [
        ...bookingPassengers,
            ...
            [s1, s2, s3, s4].forEach(num => {
                if (num <= total) {
                    ...
                    seatsInRow.push(
                        ...
                        >
                            {occupant ? num : num} {/* Show number even if occupied? Or show name? showing num keeps layout clean */}
                        </button>
                    );
                }
            });
...
const RoomingManager: React.FC<{ trip: Trip; bookings: Booking[]; clients: any[]; onSave: (data: OperationalData) => void }> = ({ trip, bookings, clients, onSave }) => {
    ...
    const [selectedPassenger, setSelectedPassenger] = useState<{id: string, name: string, bookingId: string} | null>(null);
    const [showManualForm, setShowManualForm] = useState(false);
    const [editingRoomId, setEditingRoomId] = useState<string | null>(null); // For room edits
             ...
             <div className="space-y-6">
                {!isEditingTrip ? (
                    ...
                    <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
                        <div className="flex justify-between mb-6"><h2 className="text-xl font-bold">Editor de Viagem</h2><button onClick={() => setIsEditingTrip(false)} className="text-gray-500 hover:text-gray-700">Cancelar</button></div>
                        {renderTripBuilder()}
                        <button onClick={handleTripSubmit} className="mt-6 bg-primary-600 text-white px-6 py-2 rounded-lg font-bold">Salvar Viagem</button>
                    </div>
                )}
             </div>
        )}
