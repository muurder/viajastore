      ...
      <div className="relative h-[500px] flex items-center justify-center mb-12 overflow-hidden">
        ...
        <div className="relative z-10 text-center px-4 max-w-4xl mx-auto">
          <div className="inline-flex items-center justify-center p-3 mb-6 bg-white/10 backdrop-blur-md rounded-2xl border border-white/20 shadow-xl animate-fade-in-up">
            <Compass className="text-primary-400 mr-2" size={24} />
            <span className="text-white font-semibold">Guias Especializados</span>
          </div>
          <h1 className="text-4xl md:text-6xl font-extrabold text-white mb-6 leading-tight drop-shadow-lg">
            Encontre o <span className="text-transparent bg-clip-text bg-gradient-to-r from-primary-400 to-secondary-400">Talento Ideal</span> para sua aventura
          </h1>
          ...
          <div className="relative max-w-2xl mx-auto mt-8">
            ...
            <div className="relative flex items-center bg-white rounded-full shadow-2xl p-2 pr-2">
              ...
              />
              <button className="bg-primary-600 hover:bg-primary-700 text-white px-6 py-3 rounded-full font-bold transition-all transform hover:scale-105">
                Buscar
              </button>
            </div>
          </div>
...
const TalentCard: React.FC<{ guide: Agency; whatsappLink: string }> = ({ guide, whatsappLink }) => {
  ...
  return (
    <div className="group bg-white rounded-[2rem] shadow-lg hover:shadow-2xl transition-all duration-300 border border-gray-100 overflow-hidden flex flex-col relative top-0 hover:-top-2">
      ...
      <div className="relative h-32 bg-gradient-to-r from-primary-600 to-secondary-600">
        ...
        <div className="absolute -bottom-12 left-6">
          <div className="relative">
            ...
            />
            {guide.cadastur && (
              <div className="absolute -right-2 -bottom-2 bg-green-500 text-white p-1 rounded-full border-2 border-white" title="Cadastur Verificado">
      ...
      <div className="pt-14 px-6 pb-6 flex-1 flex flex-col">
        ...
        <div className="flex flex-wrap gap-2 mb-6">
          {guide.languages?.map(lang => (
            <span key={lang} className="inline-flex items-center px-2 py-1 bg-blue-50 text-blue-700 text-xs font-bold rounded-md border border-blue-100">
              <Globe size={12} className="mr-1" /> {lang}
            </span>
          ))}
          {guide.specialties?.slice(0, 3).map(spec => (
            <span key={spec} className="inline-flex items-center px-2 py-1 bg-orange-50 text-orange-700 text-xs font-bold rounded-md border border-orange-100">
              {SPECIALTY_ICONS[spec] || <Star size={12} className="mr-1" />} {spec}
            </span>
          ))}
        </div>
        ...
        <div className="mt-auto flex gap-3">
          <Link to={`/${guide.slug}`} className="flex-1 py-3 bg-gray-100 hover:bg-gray-200 text-gray-800 font-bold rounded-xl text-center text-sm transition-colors flex items-center justify-center">
            Ver Perfil
          </Link>
          <a href={whatsappLink} target="_blank" rel="noopener noreferrer" className="flex-1 py-3 bg-gradient-to-r from-green-500 to-green-600 hover:from-green-600 hover:to-green-700 text-white font-bold rounded-xl text-center text-sm transition-all shadow-lg shadow-green-500/30 flex items-center justify-center">
